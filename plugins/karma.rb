class KarmaPlugin
  ...
  match /\b(\S+)\s*(\+\+|\-\-|\+1)(\s|$)/, :use_prefix => false
  ...
  def execute(m, nick, oper)
    ...
    synchronize(:karma) do
      ...
      if irc_user
        ...
        if (oper == "++" || oper == "+1") && source.can_increase_karma?
          target.karma_up!
          ...
          m.user.send("You have given 1 point of karma to #{irc_user.nick}. you have given #{source.given_points_today+1} points today")
        elsif oper == "--" && source.can_decrease_karma?
          target.karma_down!
          source.given_points_up!
          m.user.send("You have taken 1 point of karma from #{irc_user.nick}. you have given #{source.given_points_today+1} points today")
        end
      end
    end
  end
